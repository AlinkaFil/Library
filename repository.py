...
def lended_to_user(user):
    session = new_session()
    lended_to_user = session.query(func.count(Receiving.user_id)).filter(Receiving.returned == 0,
                                                                         Receiving.user_id == user.id) \
        .group_by(Receiving.book_id, Receiving.returned).scalar()
    return lended_to_user


def book_issued(book):
    session = new_session()
    book_issued = session.query(func.count(Receiving.book_id)).filter(Receiving.returned == 0,
